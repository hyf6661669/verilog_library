...
uint32_t recorded_vector[MAX_ERR_COUNT];
...
void fpsqrt_check(
	...
	float32_t f32_op_1;
	float64_t f64_op;
	float16_t f16_fpsqrt_res_0;
	float16_t f16_fpsqrt_res_1;
	float16_t f16_fpsqrt_res_2;
	float16_t f16_fpsqrt_res_3;
	float32_t f32_fpsqrt_res_0;
	float32_t f32_fpsqrt_res_1;
	...
	f16_op_0.v = (uint16_t)(*op_lo);
	f16_op_1.v = (uint16_t)(*op_lo >> 16);
	f16_op_2.v = (uint16_t)(*op_hi);
	f16_op_3.v = (uint16_t)(*op_hi >> 16);
	...
	if(*fp_format == 0) {
		f16_fpsqrt_res_0 = f16_sqrt(f16_op_0);
		if(fp16_is_nan(f16_fpsqrt_res_0.v))
			data_ok = ((*dut_res_lo & 0xFFFF) == fp16_defaultNaN);
		else
			data_ok = ((*dut_res_lo & 0xFFFF) == f16_fpsqrt_res_0.v);

		ref_res = f16_fpsqrt_res_0.v;

		if(*vector == 1) {
			// Check other data
			f16_fpsqrt_res_1 = f16_sqrt(f16_op_1);
			if(fp16_is_nan(f16_fpsqrt_res_1.v))
				data_ok = data_ok & ((*dut_res_lo >> 16) == fp16_defaultNaN);
			else
				data_ok = data_ok & ((*dut_res_lo >> 16) == f16_fpsqrt_res_1.v);

			f16_fpsqrt_res_2 = f16_sqrt(f16_op_2);
			if(fp16_is_nan(f16_fpsqrt_res_2.v))
				data_ok = data_ok & ((*dut_res_hi & 0xFFFF) == fp16_defaultNaN);
			else
				data_ok = data_ok & ((*dut_res_hi & 0xFFFF) == f16_fpsqrt_res_2.v);

			f16_fpsqrt_res_3 = f16_sqrt(f16_op_3);
			if(fp16_is_nan(f16_fpsqrt_res_3.v))
				data_ok = data_ok & ((*dut_res_hi >> 16) == fp16_defaultNaN);
			else
				data_ok = data_ok & ((*dut_res_hi >> 16) == f16_fpsqrt_res_3.v);

			ref_res = ref_res | ((uint64_t)(f16_fpsqrt_res_3.v) << 48) | ((uint64_t)(f16_fpsqrt_res_2.v) << 32) | ((uint64_t)(f16_fpsqrt_res_1.v) << 16);
		}

	} else if(*fp_format == 1) {
		f32_fpsqrt_res_0 = f32_sqrt(f32_op_0);
		if(fp32_is_nan(f32_fpsqrt_res_0.v))
			data_ok = (*dut_res_lo == fp32_defaultNaN);
		else
			data_ok = (*dut_res_lo == f32_fpsqrt_res_0.v);
		
		ref_res = f32_fpsqrt_res_0.v;

		if(*vector == 1) {
			f32_fpsqrt_res_1 = f32_sqrt(f32_op_1);
			if(fp32_is_nan(f32_fpsqrt_res_1.v))
				data_ok = data_ok & (*dut_res_hi == fp32_defaultNaN);
			else
				data_ok = data_ok & (*dut_res_hi == f32_fpsqrt_res_1.v);

			ref_res = ref_res | ((uint64_t)(f32_fpsqrt_res_1.v) << 32);
		}
	...
	*compare_ok = data_ok & fflags_ok;
	if(*compare_ok == 0) {
		...
		recorded_vector[*err_count] = *vector;
